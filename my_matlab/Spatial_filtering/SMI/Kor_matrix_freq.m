...
X_array = zeros(2*L, sample_per_bin);

for i = 0:L-1
    index(i+1,:) = i*sample_per_bin+2:(i+1)*sample_per_bin+1;
    X_array(2*i+1,:) = X1(index(i+1,:));
    X_array(2*i+2,:) = X2(index(i+1,:));
end 
Rxx = 1/sample_per_bin*(X_array*X_array');
Rxx_inv = inv(Rxx);
C = zeros(2*L,L);
for j = 1:L
...
for kk = 1:length(freqbin)
    ...
    for ii=1:length(phi)
        ...
        b_c(ii) =  stv*conj(W_reshaped(bandidx,:).');                
    end
    F_n(kk,:) = abs(b_n).^2; 
    F_c(kk,:) = abs(b_c).^2;
end
